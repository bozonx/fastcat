...
test.describe('Debug delete3', () => {
  test('debug fitTimelineZoom effect on clip', async ({ page, e2eProject }) => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const count = await clip(page, clipId).count();
      const allIds = await clipIds(page);
      console.log(`after ${(i+1)*500}ms - clip count: ${count}, allIds: ${allIds}`);
      if (count > 0) break;
    }
  });
});
