...
vi.mock('~/components/effects/SelectEffectModal.vue', () => ({ default: { template: '<div class="mock-select-effect"></div>', props: ['open'] } }));
vi.mock('~/components/properties/ParamsRenderer.vue', () => ({ default: { template: '<div class="mock-params-renderer"></div>' } }));
vi.mock('~/components/ui/UiModal.vue', () => ({ default: { template: '<div class="mock-modal"><slot name="body" /><slot /></div>', props: ['open'] } }));
...
vi.mock('~/effects', async (importOriginal) => {
  ...
  return {
    ...
    getVideoEffectManifest: (type: string) => {
      if (type === 'blur') return { name: 'Blur Effect', type: 'blur', defaultValues: { radius: 5 }, controls: [] };
      return null;
    }
  };
});
...
describe('EffectsEditor', () => {
  ...
  it('emits update:effects when removing an effect', async () => {
    ...
    const trashBtn = buttons.find(b => b.props('icon') === 'i-heroicons-trash' || b.props('color') === 'red');
