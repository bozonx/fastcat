...
describe('computeTrimGeometry', () => {
  ...
  const frameUs = timelineUs(40000);

  it('performs basic end edge trimming at normal speed (1.0) without quantization', () => {
    const result = computeTrimGeometry({
      edge: 'end',
      deltaUs: timelineUs(100000), // +100ms
      speed: 1.0,
      fps,
      quantizeToFrames: false,
      timelineRange: { startUs: 0, durationUs: timelineUs(400000) }, // 400ms (10 frames)
      sourceRange: { startUs: 0, durationUs: timelineUs(400000) },
      sourceDurationUs: timelineUs(1000000),
      hasFixedSourceDuration: true,
    });

    expect(result.valid).toBe(true);
    expect(result.timelineRange).toEqual({ startUs: 0, durationUs: timelineUs(500000) });
    expect(result.sourceRange).toEqual({ startUs: 0, durationUs: timelineUs(500000) });
  });

  it('performs basic start edge trimming at normal speed (1.0) without quantization', () => {
    const result = computeTrimGeometry({
      edge: 'start',
      deltaUs: timelineUs(80000), // +80ms (shrinking from start)
      speed: 1.0,
      fps,
      quantizeToFrames: false,
      timelineRange: { startUs: 0, durationUs: timelineUs(400000) },
      sourceRange: { startUs: 0, durationUs: timelineUs(400000) },
      sourceDurationUs: timelineUs(1000000),
      hasFixedSourceDuration: true,
    });
  ...
  it('clamps to source boundaries when extending past limits (fixed duration)', () => {
    const result = computeTrimGeometry({
      edge: 'end',
      deltaUs: timelineUs(800000), // extending by +800ms
      speed: 1.0,
      fps,
      quantizeToFrames: false,
      timelineRange: { startUs: 0, durationUs: timelineUs(400000) },
      sourceRange: { startUs: 0, durationUs: timelineUs(400000) },
      sourceDurationUs: timelineUs(600000), // source limit is 600ms, can only extend by 200ms
      hasFixedSourceDuration: true,
    });

    expect(result.valid).toBe(true);
    expect(result.timelineRange).toEqual({ startUs: 0, durationUs: timelineUs(600000) });
  ...
  it('applies frame quantization on timeline and matches source range at fractional speed (1.25)', () => {
    ...
    const result = computeTrimGeometry({
      edge: 'end',
      deltaUs: timelineUs(50000), // +50ms. Quantized to nearest frame (+1 frame = +40ms)
      speed: 1.25,
      fps,
      quantizeToFrames: true,
      timelineRange: { startUs: 0, durationUs: timelineUs(400000) }, // 10 frames
      sourceRange: { startUs: 0, durationUs: timelineUs(500000) },
      sourceDurationUs: timelineUs(1000000),
      hasFixedSourceDuration: true,
    });
    ...
    expect(result.timelineRange).toEqual({ startUs: 0, durationUs: timelineUs(440000) }); // 11 frames
    // sourceDeltaUs = 40ms * 1.25 = 50ms.
    expect(result.sourceRange).toEqual({ startUs: 0, durationUs: timelineUs(550000) });
  });
  ...
  it('correctly compensates quantization offset on both start and end source boundaries', () => {
    ...
    const result = computeTrimGeometry({
      edge: 'start',
      deltaUs: timelineUs(30000),
      speed: 1.5,
      fps,
      quantizeToFrames: true,
      timelineRange: { startUs: 0, durationUs: timelineUs(400000) }, // 10 frames
      sourceRange: { startUs: 0, durationUs: timelineUs(600000) },
      sourceDurationUs: timelineUs(1000000),
      hasFixedSourceDuration: true,
    });
  ...
  it('correctly handles start edge trimming with fractional speed (0.8) and non-aligned frames', () => {
    ...
    const result = computeTrimGeometry({
      edge: 'start',
      deltaUs: timelineUs(70000),
      speed: 0.8,
      fps,
      quantizeToFrames: true,
      timelineRange: { startUs: 0, durationUs: timelineUs(400000) },
      sourceRange: { startUs: 0, durationUs: timelineUs(320000) },
      sourceDurationUs: timelineUs(1000000),
      hasFixedSourceDuration: true,
    });
  ...
  it('supports reverse playback speeds (speed < 0) under quantization', () => {
    ...
    const result = computeTrimGeometry({
      edge: 'start',
      deltaUs: timelineUs(40000), // +1 frame timeline
      speed: -1.0,
      fps,
      quantizeToFrames: true,
      timelineRange: { startUs: 0, durationUs: timelineUs(400000) },
      sourceRange: { startUs: timelineUs(100000), durationUs: timelineUs(400000) }, // Source end was 500k
      sourceDurationUs: timelineUs(1000000),
      hasFixedSourceDuration: true,
    });
  ...
  it('rejects trims that shrink the clip below one frame', () => {
    const result = computeTrimGeometry({
      edge: 'start',
      deltaUs: timelineUs(390000), // leaving only 10ms (less than 40ms frame)
      speed: 1.0,
      fps,
      quantizeToFrames: true,
      timelineRange: { startUs: 0, durationUs: timelineUs(400000) },
  ...
  it('keeps static clip source start non-negative when extending the start left', () => {
    const result = computeTrimGeometry({
      edge: 'start',
      deltaUs: -timelineUs(200000),
      speed: 1.0,
      fps,
      quantizeToFrames: false,
      timelineRange: { startUs: timelineUs(500000), durationUs: timelineUs(400000) },
      sourceRange: { startUs: 0, durationUs: timelineUs(400000) },
      sourceDurationUs: timelineUs(400000),
