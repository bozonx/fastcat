...
describe('useMonitorTimeline', () => {
  ...
  it('computes workerTimelineClips and workerAudioClips correctly', () => {
    withMonitorTimeline((res, timelineStore) => {
      ...
      expect(rawWorkerTimelineClips.value[0].timelineRange.startTicks).toBe(0);
  ...
  it('computes signatures correctly', () => {
    withMonitorTimeline((res, timelineStore) => {
      ...
      timelineStore.timelineDoc.tracks[0].items[0].timelineRange.startTicks = 500;
  ...
  it('updates audio layout signature when audio fade curve or transition changes', () => {
    withMonitorTimeline((res, timelineStore) => {
            ...
            items: [
              {
                ...
                timelineRange: { startTicks: 0, durationTicks: 1_000_000 * TICKS_PER_MICROSECOND },
                sourceRange: { startTicks: 0, durationTicks: 1_000_000 * TICKS_PER_MICROSECOND },
              },
            ],
          },
        ],
      ...
      timelineStore.timelineDoc.tracks[0].items[0].transitionIn = {
        type: 'dissolve',
        durationTicks: 100_000 * TICKS_PER_MICROSECOND,
        mode: 'adjacent',
      };
