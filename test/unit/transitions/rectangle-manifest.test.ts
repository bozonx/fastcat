...
import { rectangleManifest } from '~/transitions/rectangle/manifest';
...
describe('rectangle transition blur parameter', () => {
  it('normalizes web blur to percentage range 0..20', () => {
    const normalized = rectangleManifest.normalizeParams?.({ blur: 0.5 });

    expect(normalized?.blur).toBe(0.5);
    expect(rectangleManifest.normalizeParams?.({ blur: -10 })?.blur).toBe(0);
    expect(rectangleManifest.normalizeParams?.({ blur: 100 })?.blur).toBe(20);
  });

  it('passes web blur percentage to shader as normalized UV value', () => {
    const params = rectangleManifest.normalizeParams?.({ blur: 5 });
