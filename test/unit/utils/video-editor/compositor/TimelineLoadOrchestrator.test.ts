...
describe('TimelineLoadOrchestrator.load', () => {
  it('returns empty result for empty timeline', async () => {
    ...
    expect(result.sequentialTimeUs).toBe(0);
  });
    ...
    const context = makeContext({
      timelineClipLoader: {
        describe: vi.fn().mockReturnValue({
          ...
          requestedSourceRangeDurationUs: 1_000_000,
          requestedSourceDurationUs: 1_000_000,
          speed: 1,
          startUs: 0,
    ...
    const context = makeContext({
      timelineClipLoader: {
        describe: vi.fn().mockReturnValue({
          ...
          layer: 0,
          trackId: 'track_0',
          requestedTimelineDurationUs: 5_000_000,
          requestedSourceRangeDurationUs: 5_000_000,
