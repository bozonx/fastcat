const fs = require('fs');
const path = './src/components/properties/clip/ClipTransformSection.vue';
...
const speedMultiplierReplacement = `import UiWheelSlider from '~/components/ui/UiWheelSlider.vue';
...
const mediaWidth = computed(() => props.mediaMeta?.video?.displayWidth ?? props.mediaMeta?.image?.displayWidth ?? 1920);
const mediaHeight = computed(() => props.mediaMeta?.video?.displayHeight ?? props.mediaMeta?.image?.displayHeight ?? 1080);

const cropTopPx = computed({
  get: () => Math.round((transformCropTop.value / 100) * mediaHeight.value),
  set: (val: number) => { transformCropTop.value = (val / mediaHeight.value) * 100; },
});
const cropBottomPx = computed({
  get: () => Math.round((transformCropBottom.value / 100) * mediaHeight.value),
  set: (val: number) => { transformCropBottom.value = (val / mediaHeight.value) * 100; },
});
const cropLeftPx = computed({
  get: () => Math.round((transformCropLeft.value / 100) * mediaWidth.value),
  set: (val: number) => { transformCropLeft.value = (val / mediaWidth.value) * 100; },
});
const cropRightPx = computed({
  get: () => Math.round((transformCropRight.value / 100) * mediaWidth.value),
  set: (val: number) => { transformCropRight.value = (val / mediaWidth.value) * 100; },
});
</script>`;

content = content.replace(speedPercentEndScript, speedMultiplierReplacement);
...
const newTemplate = \`<template>
  ...
  >
    ...
    <div v-if="props.canEditReversed" class="space-y-4">
      <div class="flex flex-col gap-0.5">
        ...
        <div class="flex items-center gap-2">
          ...
          />
          <button
            class="p-1 rounded hover:bg-ui-border-elevated text-ui-text-muted hover:text-ui-text flex-shrink-0"
    ...
    <div v-if="canEditTransform" class="space-y-4">
      ...
      <!-- Reflect -->
      <div class="flex items-center gap-2">
        <span class="text-xs text-ui-text-muted mr-1">{{ t('fastcat.clip.transform.reflect', 'Reflect:') }}</span>
        <UButton
      ...
      <div class="space-y-1 border-t border-ui-border pt-2 mt-2">
        ...
        <div class="grid grid-cols-2 gap-x-4 gap-y-2">
          <div class="flex flex-col gap-0.5">
            ...
            <UiWheelNumberInput v-model="cropTopPx" size="sm" :step="1" :wheel-step-multiplier="10" :min="0" :default-value="0" />
          </div>
          <div class="flex flex-col gap-0.5">
            <span class="text-xs text-ui-text-muted">{{
