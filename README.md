...
- The web compositor now supports the same multi-pass effects as the native Tauri backend, including `blur-fill`, `bloom`, and `gaussian-blur`. Master effects, adjustment-layer effects, and effects on image, text, shape, and solid clips are all processed through the WebGPU compute pipeline. Pixi render-texture extraction is normalized before compute processing so these paths work with the Pixi 8 structured pixel result.
