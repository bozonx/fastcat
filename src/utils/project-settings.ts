...
function createProjectSettingsSchema(defaults: FastCatProjectSettings) {
  const monitorSchema = z.object({
    previewResolution: z.coerce.number().int().min(1).max(4320).catch(defaults.monitor.previewResolution),
    ...
    toolbarPosition: z.enum(['top', 'bottom', 'left', 'right']).catch(defaults.monitor.toolbarPosition),
  });
  ...
  return z.object({
    ...
    project: z.object({
      ...
      audioDeclickDurationUs: z.coerce.number().min(0).max(1_000_000).catch(defaults.project.audioDeclickDurationUs),
      isAutoSettings: z.coerce.boolean().catch(defaults.project.isAutoSettings),
    }).transform((val) => {
      const isWidthHeightCustom = val.width !== defaults.project.width || val.height !== defaults.project.height;
    ...
    exportDefaults: z.object({
      encoding: z.object({
        ...
        bitrateMbps: z.coerce.number().min(0.2).max(200).catch(defaults.exportDefaults.encoding.bitrateMbps),
        excludeAudio: z.coerce.boolean().catch(defaults.exportDefaults.encoding.excludeAudio),
        audioCodec: z.enum(['aac', 'opus']).catch(defaults.exportDefaults.encoding.audioCodec),
        audioBitrateKbps: z.coerce.number().min(32).max(1024).catch(defaults.exportDefaults.encoding.audioBitrateKbps),
        bitrateMode: z.enum(['constant', 'variable']).catch(defaults.exportDefaults.encoding.bitrateMode),
        keyframeIntervalSec: z.coerce.number().min(1).max(60).catch(defaults.exportDefaults.encoding.keyframeIntervalSec),
...
export function normalizeProjectSettings(
  ...
  const mappedInput: Record<string, any> = {
    ...input,
    project: typeof input.project === 'object' ? input.project : (typeof input.export === 'object' ? input.export : {}),
    exportDefaults: {
