...
export interface Mask { path: string; inverted: boolean }
...
export type LayerKind =
  ...
  | { type: 'text'; content: string; font_family: string; font_size: number; color: [number, number, number, number]; weight: number; italic: boolean; align: 'left' | 'center' | 'right' | 'justify'; line_height: number; max_width: number | null }
...
export interface GradientStop { offset: number; rgba: [number, number, number, number] }
export interface Stroke { paint: Paint; width: number }
...
export type EffectSpec =
  ...
  | { type: 'chroma-key'; key_rgba: [number, number, number, number]; threshold: number; smoothness: number }
