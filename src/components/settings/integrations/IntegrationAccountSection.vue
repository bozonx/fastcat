...
// Shared presentation/logic for a FastCat-style OAuth integration card
// (BloggerDog, Fastcat Account, …). Each concrete section only supplies the
// runtime-config keys, the integration settings slot and its labels.
const props = defineProps<{
  /** Key inside `userSettings.integrations` holding this integration's state. */
  integrationKey: 'fastcatPublicador' | 'fastcatAccount';
  /** Display name shown in the card header. */
  title: string;
  /** i18n key for the descriptive hint under the title. */
  hintKey: string;
  /** i18n key for the connect button label. */
  connectActionKey: string;
  /** `runtimeConfig.public` key holding the API base URL. */
  apiUrlConfigKey: string;
  /** `runtimeConfig.public` key holding the UI base URL. */
  uiUrlConfigKey: string;
  /** OAuth redirect `target`/`state` value. */
  target: string;
  /** Whether to request the STT scope when connecting. */
  includeStt: boolean;
  /** Fallback message when API URL or token is missing. */
...
const integration = computed(() => workspaceStore.userSettings.integrations[props.integrationKey]);

const fastcat = computed<FastCatPublicadorIntegrationSettings | undefined>(
  () => workspaceStore.userSettings?.integrations?.[props.integrationKey],
);

function readConfigUrl(key: string) {
  const value = (runtimeConfig.public as Record<string, unknown>)[key];
  return typeof value === 'string' ? value.trim() : '';
}

const apiUrl = computed(() => readConfigUrl(props.apiUrlConfigKey));
const uiUrl = computed(() => readConfigUrl(props.uiUrlConfigKey));

const redirectUri = computed(() => {
  if (typeof window === 'undefined') return '';
  return `${window.location.origin}${route.path}?target=${props.target}`;
});

const fastcatConnectUrl = computed(() => {
  if (!uiUrl.value) return '';
...
function disconnectFastCat() {
  integration.value.enabled = false;
  integration.value.bearerToken = '';
...
<template>
  <div class="rounded-lg border border-ui-border p-4 flex flex-col gap-4">
    <div class="flex items-start justify-between gap-4">
      <div class="min-w-0">
        <div class="text-sm font-medium text-ui-text">{{ title }}</div>
    ...
    <div v-if="!fastcat?.bearerToken" class="flex flex-col gap-4">
      <div class="flex items-center gap-3">
        <UButton color="primary" variant="solid" :disabled="!uiUrl" @click="startFastCatConnect">
          {{ t(connectActionKey) }}
        </UButton>
      </div>
    </div>
