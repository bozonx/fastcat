...
<template>
  <div 
    class="w-full flex"
    :class="[
      vertical ? 'flex-col gap-2' : 'flex-wrap items-center gap-1',
      props.class
    ]"
  >
    <UButton
      ...
      :label="action.label"
      :icon="action.icon"
      :color="(action.color === 'danger' ? 'error' : action.color) || (props.color === 'danger' ? 'error' : props.color) || 'neutral'"
