...
<template>
  <div class="flex items-center gap-2 border-b border-zinc-800 bg-zinc-900/50 px-3 py-2.5">
    <UButton
    ...
    <div class="flex-1 overflow-x-hidden">
      ...
      <div
        v-else
        class="flex items-center gap-1 text-xs text-zinc-400 overflow-x-auto no-scrollbar"
      >
        <button
          class="shrink-0 transition-colors py-1 px-1.5 -ml-1 rounded-md hover:bg-zinc-800 hover:text-zinc-100 truncate max-w-[100px]"
        ...
        <template v-for="bc in breadcrumbs" :key="bc.path">
          ...
          <button
            class="shrink-0 transition-colors py-1 px-1.5 rounded-md hover:bg-zinc-800 hover:text-zinc-100 last:text-zinc-100 last:font-medium truncate max-w-[100px]"
