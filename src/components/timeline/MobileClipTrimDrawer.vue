...
function onMove(e: TouchEvent) {
  ...
  if (Math.abs(diffUs) > 1000) { // Apply every 1ms change
    try {
      ...
      const nextTimeUs = activeEdge.value === 'start'
        ? clip.value.timelineRange.startUs
        : clip.value.timelineRange.startUs + clip.value.timelineRange.durationUs;
...
<template>
  ...
  >
    <template #header>
      <div class="flex items-center gap-2">
        ...
        <div class="flex flex-col">
          ...
          <span v-if="clip" class="text-[10px] text-slate-500 mt-1 uppercase tracking-tighter">
    ...
    <div class="px-4 py-8">
      <div class="bg-slate-900/60 border border-slate-800/80 rounded-2xl overflow-hidden shadow-inner">
        <div class="flex h-44 divide-x divide-slate-800/80">
          <!-- Start Area -->
          <div
          ...
          >
            ...
            <UIcon
              ...
              :class="activeEdge === 'start' ? 'text-blue-400 scale-125' : 'text-slate-600'"
            />

            <div class="flex flex-col items-center">
              <span class="text-[10px] uppercase tracking-[0.2em] font-black text-slate-500">
            ...
            <div class="w-full flex justify-center gap-2 overflow-hidden px-4">
              ...
              >
                <div
                  ...
                  :class="[
                    i % 5 === 0 ? 'h-6' : 'h-3',
                    activeEdge === 'start' ? 'bg-blue-400/40' : 'bg-slate-700/30'
                  ]"
                ></div>
              </div>
            </div>
          ...
          >
            ...
            <UIcon
              ...
              class="w-5 h-5 transition-transform"
              :class="activeEdge === 'end' ? 'text-blue-400 scale-125' : 'text-slate-600'"
            />
            ...
            <div class="w-full flex justify-center gap-2 overflow-hidden px-4">
              ...
              >
                <div
                  ...
                  :class="[
                    i % 5 === 0 ? 'h-6' : 'h-3',
                    activeEdge === 'end' ? 'bg-blue-400/40' : 'bg-slate-700/30'
                  ]"
                ></div>
              </div>
            </div>
        ...
        <!-- Hint -->
        <div class="bg-slate-900 border-t border-slate-800/80 py-2 px-4 flex justify-center items-center gap-2">
          <UIcon name="i-heroicons-hand-raised" class="w-3.5 h-3.5 text-slate-600" />
          <span class="text-[10px] text-slate-500 font-medium uppercase tracking-widest">
    ...
    <template #footer>
      <div class="flex gap-3">
        <button
          class="flex-1 bg-slate-800 hover:bg-slate-700 active:bg-slate-600 text-slate-200 py-3 rounded-xl font-bold text-sm transition-all shadow-lg active:scale-95"
          @click="close"
        >
          {{ t('common.done') }}
        </button>
