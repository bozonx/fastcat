...
///
/// * `hw_mode` – `"auto"`, `"vaapi"`, `"videotoolbox"`, `"d3d11va"`, `"none"` or `None`.
...
pub fn init_hwaccel(
    ...
    let mode = hw_mode.unwrap_or("none");
    if mode == "none" {
        return None;
    }
    ...
    #[cfg(target_os = "macos")]
    if mode == "auto" || mode == "videotoolbox" {
...
pub fn try_transfer_to_cpu(
    ...
    let sw_ptr = unsafe { sw_frame.as_mut_ptr() as *mut ffmpeg_sys_next::AVFrame };
