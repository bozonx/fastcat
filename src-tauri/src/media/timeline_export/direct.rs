...
fn evaluate_direct(
    ...
    if layer.timeline_start_sec > start + EPS || layer.timeline_end_sec < end - EPS {
        return Err(format!(
            "clip [{:.3}s,{:.3}s] does not cover the export range [{start:.3}s,{end:.3}s]",
            layer.timeline_start_sec, layer.timeline_end_sec
        ));
    }
    ...
    if (src_aspect - out_aspect).abs() > out_aspect * 0.005 {
        return Err(format!(
            "source aspect {src_aspect:.4} ({}x{}) differs from output aspect {out_aspect:.4} ({width}x{height}) — would need letterboxing",
