...
describe('UiTimecode', () => {
  ...
  it('formats partial seconds to frames correctly', async () => {
    const component = await mountSuspended(UiTimecode, {
      props: {
        modelValue: (254_016_000_000 / 30) * 5, // exactly 5 frames
      },
    });
  ...
  it('emits update:modelValue with parsed ticks on Enter', async () => {
    ...
    expect(component.emitted('update:modelValue')?.[0]).toEqual([635_040_000_000]);
  });
  ...
  it('steps value up and down via up/down buttons', async () => {
    ...
    expect(component.emitted('update:modelValue')?.[0]).toEqual([
      254_016_000_000 + 254_016_000_000 / 30,
    ]);
  ...
  it('does not update localValue from props when focused', async () => {
    ...
    await component.setProps({ modelValue: 508_032_000_000 });
  ...
  describe('bounds (min/max)', () => {
    const frameTicks = 254_016_000_000 / 30;

    it('clamps manual entry to max on Enter', async () => {
      const component = await mountSuspended(UiTimecode, {
        props: { modelValue: 254_016_000_000, max: 508_032_000_000 },
      });
      ...
      expect(component.emitted('update:modelValue')?.[0]).toEqual([508_032_000_000]);
      expect((input.element as HTMLInputElement).value).toBe('00:00:02:00');
    });

    it('clamps manual entry to min on Enter', async () => {
      const component = await mountSuspended(UiTimecode, {
        props: { modelValue: 254_016_000_000, min: 127_008_000_000 },
      });
      ...
      expect(component.emitted('update:modelValue')?.[0]).toEqual([127_008_000_000]);
    });

    it('clamps step (up button) to max', async () => {
      const component = await mountSuspended(UiTimecode, {
        props: { modelValue: 505_491_840_000, max: 508_032_000_000 },
      });
    ...
    it('does not clamp when max is Infinity/omitted', async () => {
      const component = await mountSuspended(UiTimecode, {
        props: { modelValue: 254_016_000_000 },
      });
      ...
      expect(component.emitted('update:modelValue')?.[0]).toEqual([152_409_600_000_000]);
    });
    ...
    it('allows negative values when allowNegative is true and no min', async () => {
      ...
      expect(component.emitted('update:modelValue')?.[0]).toEqual([-254_016_000_000]);
    });
  });
});
