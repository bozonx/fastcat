...
describe('findNextMarkerTime', () => {
  ...
  it('jumps from start to a point marker', () => {
    const markers = [makeMarker(254_016_000_000)];
    expect(findNextMarkerTime(markers, 0, FPS)).toBe(254_016_000_000);
  });

  it('jumps over a zone start to its end when playhead is before the zone', () => {
    const markers = [makeMarker(127_008_000_000, 127_008_000_000)];
    expect(findNextMarkerTime(markers, 0, FPS)).toBe(127_008_000_000);
  });

  it('jumps from inside a zone to its end', () => {
    const markers = [makeMarker(0, 254_016_000_000)];
    expect(findNextMarkerTime(markers, 25_401_600_000, FPS)).toBe(254_016_000_000);
  });

  it('jumps from zone end to next marker', () => {
    const markers = [makeMarker(0, 254_016_000_000), makeMarker(508_032_000_000)];
    expect(findNextMarkerTime(markers, 254_016_000_000, FPS)).toBe(508_032_000_000);
  });

  it('skips duplicate boundary points (zone end == next start)', () => {
    const markers = [makeMarker(0, 254_016_000_000), makeMarker(254_016_000_000)];
    expect(findNextMarkerTime(markers, 0, FPS)).toBe(254_016_000_000);
    // after landing on 1_000_000, next jump should find nothing (single unique point)
    expect(findNextMarkerTime(markers, 254_016_000_000, FPS)).toBeUndefined();
  });

  it('handles multiple markers with same start time', () => {
    const markers = [makeMarker(0), makeMarker(0), makeMarker(254_016_000_000)];
    expect(findNextMarkerTime(markers, 0, FPS)).toBe(254_016_000_000);
  });

  it('does not get stuck when currentTime is slightly off due to frame quantization', () => {
    const markers = [makeMarker(254_016_000_000), makeMarker(508_032_000_000)];
  ...
  it('handles non-30fps quantization correctly', () => {
    ...
    const markers = [makeMarker(254_016_000_000), makeMarker(508_032_000_000)];
    expect(findNextMarkerTime(markers, 254_016_000_000, 25)).toBe(508_032_000_000);
  });
});
...
describe('findPreviousMarkerTime', () => {
  ...
  it('jumps back to a point marker', () => {
    const markers = [makeMarker(254_016_000_000)];
    expect(findPreviousMarkerTime(markers, 508_032_000_000, FPS)).toBe(254_016_000_000);
  });
  ...
  it('jumps from inside a zone back to its start', () => {
    const markers = [makeMarker(0, 254_016_000_000)];
    expect(findPreviousMarkerTime(markers, 127_008_000_000, FPS)).toBe(0);
  });

  it('jumps from zone end to zone start', () => {
    const markers = [makeMarker(0, 254_016_000_000)];
    expect(findPreviousMarkerTime(markers, 254_016_000_000, FPS)).toBe(0);
  });
