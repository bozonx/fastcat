...
describe('@bozonx/fastcat-embed package build and distribution smoke test', () => {
  beforeAll(() => {
    // Ensure dist is built before running assertions
    execSync('pnpm --filter @bozonx/fastcat-embed build', { stdio: 'pipe', cwd: ROOT });
  });
  ...
  it('validates package.json exports, types, files and engines fields', () => {
    ...
    expect(pkg.name).toBe('@bozonx/fastcat-embed');
